...
// Exported methods
Nexo.prototype.sendNextMessage = function () {
    ...
    if (this.message_queue.length > 0) {
      ...
      queue_debug('sendNextMessage ' + util.inspect(message));
...
Nexo.prototype.scheduleMessage = function(priority, payload, read_mode, callback) {
    ...
    if (this.state WHAT SHOUD IT BE? CURRENTLY NOT WAITING FOR AMY DATA) this.sendNextMessage();
}
...
function nexoOnData(data) {
    ...
    if (data=='Welcome to AISIS server.') {
        ...
        this.write('1234'); //new Buffer(hex2a(this.pass_md5 || require('crypto').createHash('md5').update(this.password).digest('hex'))));
    } else if (data == 'LOGIN FAILED') {
        this.destroy();
    } else if (data == 'LOGIN OK') {
        this.write('!\0');
        this.encodeAndWrite(true, 'system info nexo-node connected!', false, null); // manage callback here
