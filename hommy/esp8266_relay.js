...
ESP_Relay.prototype.sync = function() {
  // Read current state
  this.is_on = this.http_get( 'relay' );
...
ESP_Relay.prototype.http_get = function( uri, param ) {
  http.get( this.get_url(uri, param), function (res) {
    debug( '[ESP_DHT21 response: ' + res.statusCode );
    res.on('data', function (chunk) {
      debug( '[ESP_DHT21] data: ' + chunk );
      return JSON.parse(chunk).relay;
    });
  })
  .on( 'error', function (err) {
    debug( '[ESP_DHT21] error: ' + err.message );
    return undefined;
  })
}
